...
import com.jitendra.Wallet.services.saga.SagaOrchestrator;
...
public class TransactionService {
     ...
     * @return TransactionResponseDTO with transaction details including saga instance ID
     */
    @Transactional
    public TransactionResponseDTO createTransaction(TransactionRequestDTO transactionRequest) {
        ...
        Wallet sourceWallet = walletRepository.findById(transactionRequest.getSourceWalletId())
            .orElseThrow(() -> new RuntimeException("Source wallet not found with id: " + transactionRequest.getSourceWalletId()));
        
        // Validate destination wallet exists
        if (!walletRepository.existsById(transactionRequest.getDestinationWalletId())) {
            throw new RuntimeException("Destination wallet not found with id: " + transactionRequest.getDestinationWalletId());
        }
        ...
        // Create saga context for transaction and put request data
        SagaContext sagaContext = SagaContext.builder()
            .sagaType("TRANSACTION_TRANSFER")
            .build();
        sagaContext.put("transactionRequest", transactionRequest);
        
        // Start saga orchestration
        Long sagaInstanceId = sagaOrchestrator.startSaga(sagaContext);
        
        // Create transaction entity using builder pattern
        ...
        log.info("Transaction created successfully with id: {} and saga instance id: {}", 
    ...
    /**
     * Update transaction status (used by saga orchestrator during compensation or completion)
     * 
     ...
     * @return List of TransactionResponseDTO
     */
    public List<TransactionResponseDTO> getTransactionsBetweenWallets(Long sourceWalletId, Long destinationWalletId) {
        log.info("Fetching transactions between source wallet id: {} and destination wallet id: {}", 
                 sourceWalletId, destinationWalletId);
        List<Transaction> transactions = transactionRepository.findBySourceWalletIdAndDestinationWalletId(sourceWalletId, destinationWalletId);
        return transactions.stream()
            .map(this::mapToResponseDTO)
            .toList();
    }
    ...
    public List<TransactionResponseDTO> getPendingTransactionsBySagaInstance(Long sagaInstanceId) {
        ...
        List<Transaction> transactions = transactionRepository.findBySagaInstanceIdAndStatus(sagaInstanceId, TransactionStatus.PENDING.name());
        return transactions.stream()
            .map(this::mapToResponseDTO)
            .toList();
    }
