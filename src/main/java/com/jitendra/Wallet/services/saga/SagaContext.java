 ...
 * Shared across all saga steps and contains input parameters, intermediate
 * results, and metadata.
 * 
 * This context is serialized to JSON and stored in the saga_instance table for
 * persistence.
 * 
 * Usage examples:
 * - Simple: new SagaContext()
 * - With builder:
 * SagaContext.builder().sagaType("USER_REGISTRATION").sagaInstanceId(123L).build()
 */
@Data
@Builder
@NoArgsConstructor
...
public class SagaContext {
    ...
    // Type/name of the saga workflow (e.g., "USER_REGISTRATION",
    // "PAYMENT_TRANSFER")
    private String sagaType;

    // Storage for saga data - key-value pairs holding input parameters and step
    // results
    @Builder.Default
    private Map<String, Object> data = new HashMap<>();

    // Metadata about the saga execution (timestamps, user info, correlation IDs,
    // etc.)
    @Builder.Default
